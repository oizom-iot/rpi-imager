...
Popup {
    ...
    ColumnLayout {
            ...
            GroupBox {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        CheckBox {
                            id: chknametxt
                            text: qsTr("Set name.txt:")
                            onCheckedChanged: {
                                if (checked) {
                                    fieldnametxt.forceActiveFocus()
                                }
                            }
                        }
                        TextField {
                            id: fieldnametxt
                            enabled: chknametxt.checked
                            text: "raspberrypi"
                        }
                    }
                    CheckBox {
        ...
        RowLayout {
            ...
            Button {
                ...
                Material.background: "#2CAF96"//c51a4a
                font.family: roboto.name
    ...
    function initialize() {
        ...
        if ('nametxt' in settings) {
            fieldnametxt.text = settings.nametxt
            chknametxt.checked = true
        }
    ...
    function applySettings()
    {
        ...
        if (chknametxt.checked && fieldnametxt.length) {
            addFirstRun("CURRENT_HOSTNAME=`cat /etc/hostname | tr -d \" \\t\\n\\r\"`")
            addFirstRun("echo "+fieldnametxt.text+" >/etc/name.txt")
            addFirstRun("sed -i \"s/127.0.1.1.*$CURRENT_HOSTNAME/127.0.1.1\\t"+fieldnametxt.text+"/g\" /etc/hosts")
        }
        if (chkSSH.checked) {
    ...
    function saveSettings()
    {
        if (comboSaveSettings.currentIndex == 1) {
            ...
            if (chknametxt.checked && fieldnametxt.length) {
                settings.nametxt = fieldnametxt.text
            }
            if (chkSSH.checked) {
                if (radioPasswordAuthentication.checked) {
